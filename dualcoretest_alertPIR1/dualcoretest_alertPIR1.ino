...
const uint8_t MIN_COUNTER_FOR_ALERT = 2;        // Minimum counter for alert
...
// Counter-based motion detection task with 6-second polling
void motionDetectionTask(void *pvParameters) {
    ...
    while(1) {
        ...
        // Control status LED
        digitalWrite(STATUS_LED_PIN, motionActive ? HIGH : LOW);
...
void setup() {
    Serial.begin(115200);
    Serial.println("ESP32 6-Second Polling Motion Detection System");
    ...
    Serial.println("6-Second Polling Motion Detection System Initialized");
    ...
    Serial.println("- Alert levels: 3=Slow, 6=Medium, 10=Fast, 15=Critical");
    
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("System Ready");
    lcd.setCursor(0, 1);
    lcd.print("6s Cycle Mode");
    delay(1000);
}
